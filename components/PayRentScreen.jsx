...
function calcLateFee(dayOfMonth) {
  ...
  return 35 + Math.max(0, (dayOfMonth - 4) - 1) * 10;
}
    ...
    <div style={{ padding: 16, fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          {invoices.map(inv => (
            <button key={inv.id} onClick={() => setSelectedInvoiceId(inv.id)} style={{
              width: "100%", marginBottom: 8, padding: "12px 16px", borderRadius: 10, cursor: "pointer", textAlign: "left",
              ...
              <div>
                <div style={{ fontSize: 14, fontWeight: 700 }}>{inv.month}</div>
                {Number(inv.late_fee) > 0 && <div style={{ fontSize: 12, color: "#dc2626" }}>$35 base + ${Number(inv.late_fee) - 35} accrued late fees</div>}
              </div>
      ...
      {payMode === "current" && (dayOfMonth < 5 && daysLeft > 0 ? (
        ...
        <div style={{ background: "#fef2f2", border: "1px solid #fca5a5", borderRadius: 12, padding: "12px 16px", marginBottom: 14, fontSize: 13, color: "#991b1b" }}>
          ⚠️ <strong>Late fee applied.</strong> Total due includes <strong>${invoiceLateFee} in late fees</strong>
        </div>
      ) : null)}
      ...
      {step === "checkout" && method === "ach" && (
        <div style={{ background: "#fff", borderRadius: 14, padding: "18px", border: "1px solid rgba(0,0,0,0.07)", marginBottom: 14 }}>
          ...
          <div style={{ fontSize: 12, color: "#6b7280", background: "#f9fafb", borderRadius: 8, padding: "8px 12px", marginBottom: 14 }}>ACH transfers take 3–5 business days. Your payment date is today.</div>
