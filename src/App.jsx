...
import { saveTenants, loadTenants, saveTickets, loadTickets } from "./storage";
...
const INITIAL_TENANTS = [
  {
    ...
    rent: 900, paid: false,
    overrideLate: 285, section8: false,
    email: "", phone: "",
    leaseStart: "", leaseEnd: "",
...
export default function App() {
  ...
  const [tickets, setTickets] = useState(() => {
    try { const s = localStorage.getItem("gi_tickets"); return s ? JSON.parse(s) : []; } catch(e) { return []; }
  });
  const [tenants, setTenants] = useState(() => {
    try { const s = localStorage.getItem("gi_tenants"); return s ? JSON.parse(s) : INITIAL_TENANTS; } catch(e) { return INITIAL_TENANTS; }
  });
  ...
  // Auto-save tenants and tickets to localStorage on every change
  ...
  useEffect(() => {
    try { localStorage.setItem("gi_tenants", JSON.stringify(tenants)); } catch(e) {}
  }, [tenants]);

  useEffect(() => {
    try { localStorage.setItem("gi_tickets", JSON.stringify(tickets)); } catch(e) {}
  }, [tickets]);
