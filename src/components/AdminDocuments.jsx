...
export default function AdminDocuments({ tenants, setTenants }) {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      // Save to tenant's documents array in Supabase
      ...
      setTenants(tenants.map(t =>
        String(t.id) === String(form.tenantId) ? { ...t, documents: updatedDocs } : t
      ));
  ...
  const handleRemove = async (tenantId, docId) => {
    ...
    setTenants(tenants.map(t =>
      String(t.id) === String(tenantId) ? { ...t, documents: updatedDocs } : t
    ));
  };
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            <div
              ...
              style={{
                ...
                textAlign: "center",
                cursor: "pointer",
                background: dragging ? "#f0f9f4" : file ? "#f0fdf4" : "#fafafa",
                transition: "all 0.2s",
              }}>
            ...
            {uploadError && (
              <div style={{ fontSize: 12, color: "#dc2626", marginTop: 6 }}>⚠️ {uploadError}</div>
            )}
          </div>
          ...
          {saved ? (
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={handleAdd} disabled={uploading || !form.name || !file || !form.tenantId} style={{
                background: (!form.name || !file || !form.tenantId) ? "#d1d5db" : "#1b3d2a",
                color: "#fff", border: "none", borderRadius: 10, padding: "12px 24px",
                fontSize: 14, fontWeight: 700,
                cursor: (!form.name || !file || !form.tenantId) ? "not-allowed" : "pointer",
