export default function Dashboard({ tenant, onTabClick, onLogout }) {
  function calcLateFee(paid) { if (paid) return 0; const day = new Date().getDate(); if (day < 5) return 0; return 35 + Math.max(0, day - 5) * 10; }
  const rent = Number(tenant.rent) || 0;
  const lateFee = tenant.section8 ? 0 : (tenant.overrideLate != null ? tenant.overrideLate : calcLateFee(tenant.paid));
