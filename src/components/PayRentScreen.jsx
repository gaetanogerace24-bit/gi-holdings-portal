...
// rules = { startDay, initialFee, dailyFee } — pass tenant's custom values or leave undefined for global defaults
function calcLateFee(dueDateStr, rules = {}) {
  ...
  const initialFee = Number(rules.initialFee) ?? 35;
  const dailyFee = Number(rules.dailyFee) ?? 10;
...
export default function PayRentScreen({ tenant, invoices = [], onPaymentSuccess, defaultPayMode = "current" }) {
  ...
  // Per-tenant late fee rules — fall back to global defaults (5th, $35, $10) when not set
  const lateFeeRules = tenant?.custom_late_fee ? {
    startDay: Number(tenant.late_fee_start_day) || 5,
    initialFee: Number(tenant.initial_late_fee) ?? 35,
    dailyFee: Number(tenant.daily_late_fee) ?? 10,
  } : {};
