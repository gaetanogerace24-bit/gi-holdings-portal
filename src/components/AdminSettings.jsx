    ...
    <div style={{ padding: 28, maxWidth: 740, fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <Section title="💰 Rent & late fee rules">
        <div style={{ background: "#f0f9f4", border: "1px solid #bbf7d0", borderRadius: 10, padding: "12px 16px", marginBottom: 16, fontSize: 13, color: "#166534" }}>
          <strong>Current rules:</strong> Rent due the {settings.rentDueDay}{settings.rentDueDay === "1" ? "st" : settings.rentDueDay === "2" ? "nd" : settings.rentDueDay === "3" ? "rd" : "th"} · $
          {settings.initialLateFee} late fee kicks in at midnight {settings.lateFeeDay} · then $
        ...
        <Grid>
          ...
          <div>
            ...
            <select value={settings.lateFeeDay} onChange={e => update("lateFeeDay", e.target.value)} style={selectSt}>
              {["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"].map(d => <option key={d}>{d}</option>)}
            </select>
            <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 4 }}>Day of week after due date</div>
          </div>
          <Field label="Initial late fee ($)" value={settings.initialLateFee} onChange={v => update("initialLateFee", v)} type="number" hint="First charge when late" />
          <Field label="Daily late fee ($)" value={settings.dailyLateFee} onChange={v => update("dailyLateFee", v)} type="number" hint="Per day until paid" />
        </Grid>
      </Section>
