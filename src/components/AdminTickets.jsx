...
export default function AdminTickets({ tenants }) {
  const [tickets, setTickets] = useState([]);
  const [filter, setFilter] = useState("all");
  const [showAdd, setShowAdd] = useState(false);
  const [form, setForm] = useState({ tenantId: "", title: "", category: "General", urgency: "medium", notes: "" });

  const updateStatus = (id, status) => setTickets(tickets.map(t => t.id === id ? { ...t, status } : t));
  ...
  const handleRemove = (id) => {
    if (window.confirm("Delete this ticket?")) setTickets(tickets.filter(t => t.id !== id));
  };
      ...
      <div style={{ marginBottom: 24, display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
        <div>
          ...
          <div style={{ fontSize: 14, color: "#6b7280", marginTop: 4 }}>
            {tickets.length === 0 ? "No tickets yet" : `${activeCount} active · ${tickets.filter(t => t.status === "resolved").length} resolved`}
          </div>
        </div>
        {tenants.length > 0 && (
      ...
      {tickets.length > 0 && (
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          {["all", "open", "in-progress", "resolved"].map(f => (
            ...
            }}>{f === "all" ? `All (${tickets.length})` : STATUS_STYLE[f].label}</button>
          ))}
        </div>
      )}
